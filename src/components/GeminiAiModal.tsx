...
import { Sparkles, X, Loader2, AlertCircle, Upload, RefreshCw, CheckCircle2, Trash2 } from 'lucide-react';
...
export const GeminiAiModal: React.FC<GeminiAiModalProps> = ({
  ...
  // Clipboard paste handler for pasting images directly
  useEffect(() => {
    if (!isOpen) return;
      ...
      <div className="bg-slate-900 border border-white/10 rounded-2xl max-w-2xl w-full p-5 sm:p-6 shadow-2xl relative overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between pb-4 border-b border-white/10 shrink-0">
          <div className="flex items-center gap-3">
            <div className="p-3 rounded-2xl bg-indigo-600 text-white shadow-[0_0_20px_rgba(79,70,229,0.5)] border border-indigo-400/40">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-slate-300 font-mono font-medium">
                Gemini 3.6 Flash yordamida rasm yoki mavzudan savol generatsiyasi
              </p>
            </div>
          </div>
          ...
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-white p-2 rounded-2xl hover:bg-slate-800 transition-colors cursor-pointer"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        <div className="overflow-y-auto py-4 space-y-5 pr-1 flex-1">
          {error && (
            <div className="p-4 rounded-2xl bg-rose-500/15 border border-rose-500/30 text-rose-200 text-xs sm:text-sm font-bold flex items-center gap-2.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="sm:col-span-2">
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-2xl bg-slate-950 border border-white/10 text-white font-semibold text-sm sm:text-base focus:outline-none focus:border-indigo-500"
              />
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-4 py-3 rounded-2xl bg-slate-950 border border-white/10 text-white font-bold text-sm sm:text-base focus:outline-none focus:border-indigo-500"
              >
          ...
          <div>
            <label className="block text-xs font-extrabold uppercase tracking-widest text-slate-300 mb-2">
            ...
            <div className="grid grid-cols-3 gap-3">
              <button
                ...
                className={`py-3 px-4 rounded-2xl border text-xs sm:text-sm font-bold transition-all flex items-center justify-center gap-2 cursor-pointer ${
                  difficulty === 'Oson'
            ...
            <div className="mt-4 pt-4 border-t border-white/10 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  disabled={loading}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-600/30 hover:bg-indigo-600/50 text-indigo-200 border border-indigo-500/40 text-[11px] font-bold uppercase transition-all"
                  title="Xuddi shu so'rov bo'yicha yana 10 ta yangi savol generatsiya qilish"
                >
                  <RefreshCw className={`w-3.5 h-3.5 text-indigo-400 ${loading ? 'animate-spin' : ''}`} />
                  Yangi {count} ta savol olis
                </button>
              </div>
        ...
        <div className="flex items-center justify-between gap-3 pt-4 border-t border-white/10 shrink-0">
          ...
          <div className="flex items-center gap-2">
            {/* Refresh / Re-generate Icon Button */}
            <button
              onClick={handleGenerate}
              disabled={loading}
